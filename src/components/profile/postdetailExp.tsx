...
import FeedLayout from '../screens/rewampedExp/feedLayout';
import { toggleLike } from '../../redux/slices/likeSlice';
import { setSaveStatus } from '../../redux/slices/saveSlice';
import { RootState } from '../../redux/store';
import { updatePostFollowStatus } from '../../redux/slices/feedSlice';
...
import { handleSinglePostShare } from '../screens/jobs/utils/utils';    
...
import CommentInputCard from '../screens/Home/CommentInputCard';
...
const PostDetailExp = ({ route, navigation: propNavigation }: { route: { params: RouteParams }, navigation: any }) => {
  ...
  const handleTap = () => {
    ...
    if (filteredPosts.length > 0 && !isCurrentPostFiltered) {
      const items = filteredPosts.map(post => {
        ...
        console.log("posterDetails :::", posterDetails?.firstName);
  ...
  const toggleFullScreen = () => {
    if (currentPost?.contentType === 'video') {
      ...
      if (currentVideoIndex >= 0) {
        ...
        const posterDetails = currentPost.posterDetails;
        console.log("posterDetails::::::::", posterDetails);
        navigation.navigate('VideoFullScreenRewamped', {
          items: [{
            ...
            userDetails: {
              ...
              accountType: currentPost?.posterDetails?.accountType || ''
            },
 ...
 const loggedInUserId =useCurrentUserId()
  ...
  useEffect(() => {
    if (currentPost?._id && commentCounts && commentCounts[currentPost._id]) {
      const reduxCommentCount = commentCounts[currentPost._id] || 0;
      // Update local state with Redux comment count
      if (currentPost.commentsCount !== reduxCommentCount) {
        setCurrentPost(prev => prev ? {
          ...prev,
          commentsCount: reduxCommentCount
        } : null);
      }
    }
