...
const FollowingList = ({ route }) => {
  ...
  const fetchFollowingList = async (userid, page) => {
    try {
      setLoading(true);
      const response = await getWithoutToken(`user/get-user-following/${userid}?page=${page}&limit=20`, {});
      if (response && response.status === 200) {
        setFollowingData(prevData => ({
          ...response,
          following: page === 1 ? response.following : [...prevData.following, ...response.following],
        }));
      } else {
  ...
  const renderFooter = () => {
    if (loading && followingData.currentPage === 1 && followingData.following.length === 0) {
    ...
    } else if (followingData.following.length === 0) {
      // Empty list message
      return (
        <View style={styles.loaderContainer}>
