...
const DeleteRoomModal: React.FC<DeleteRoomModalProps> = ({
  ...
  const handleDelete = () => {
    ...
    payload.append('room_id', roomData.id);
    payload.append('created_at', roomData.created_at);
    console.log('block payload', payload);

    chatRequest(
      apiEndPoints.deleteChat(roomData?.id),
      'POST',
      payload,
      'multipart/form-data',
    )
      .then((res: any) => {
        console.log('delete room', res,payload, apiEndPoints.deleteChat(roomData?.id),);
        if (!res?.error) {
          ...
          console.log('delete ', res?.error_messages);
          setVisible(false);
        }
      })
      .catch(e => {
